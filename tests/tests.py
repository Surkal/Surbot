...
class TestSortkey(TestCase):
    ...
    def test_add_sortkey(self):
        t = sortkey(self.f, self.page, self.text, 'sv')
        ...
        for x in self.langs:
            self.text = sortkey(self.f, self.page, self.text, x)
        assert self.text == p

    def test_no_sortkey(self):
        # Wrong section
        t = sortkey(self.f, self.page, self.text, 'sv', section='dérivés')
        assert t == self.text
